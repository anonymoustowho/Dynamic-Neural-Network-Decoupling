...
def train_architecture_decouple(train_loader, model, criterion, optimizer, epoch, writer, Lambda_k, Lambda_s, Lambda_m, net):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
        ...
        if "resnet56" in net:
            for k in range(1, 4):
                for j in range(9):
                    ...
                    s_loss += torch.mean(torch.norm(mask1, 1, 1)) + torch.mean(torch.norm(mask1, 2, 0))
                    ...
                    s_loss += torch.mean(torch.norm(mask2, 1, 1)) + torch.mean(torch.norm(mask2, 2, 0))
            ...
            for k in range(1, 4):
                for j in range(3):
                    ...
                    s_loss += torch.mean(torch.norm(mask, 1, 1))
            ...
            for k in range(1, 5):
                for j in range(2):
                    ...
                    m_loss += criterion(cls1, target)
                    kl_loss += F.kl_div(F.log_softmax(mask1, 1), F.softmax(out1, 1))
                    s_loss += torch.mean(torch.norm(mask1, 1, 1))
                    ...
                    m_loss += criterion(cls2, target)
                    kl_loss += F.kl_div(F.log_softmax(mask2, 1), F.softmax(out2, 1))
                    s_loss += torch.mean(torch.norm(mask2, 1, 1))
                    ...
                    if model.__getattr__("layer" + str(k))[j].downsample is not None:
                        ...
                        s_loss += torch.mean(torch.norm(mask3, 1, 1)) * 0.3
